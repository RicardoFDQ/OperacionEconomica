    ...
    function modelo_P2()
        ...
        for barra in barras
            for bloque in 1:length(barra.demanda)
                ...
                (barra.demanda[bloque]/100) + sum(linea.reactancia^(-1) * (θ[barra.id, bloque] - θ[linea.barra_fin, bloque]) for linea in lineas_out_barras_P2[barra.id]; init = 0)
